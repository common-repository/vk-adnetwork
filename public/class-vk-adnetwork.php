 ...
 *
 * Класс плагина. В идеале этот класс следует использовать для работы с
 * общедоступной частью сайта WordPress.
 *
...
class VK_Adnetwork {
     ...
     * @var     string
     */
    const POST_TYPE_SLUG = 'vk_adnetwork';
    ...
    /**
     * Loaded instance of VK_Adnetwork_Model
     *
     * @var VK_Adnetwork_Model
     */
    protected $model;

    /**
     * Loaded instance of VK_Adnetwork_Plugin
     *
    ...
    /**
     * Loaded instance of VK_Adnetwork_Select
     *
    ...
    private function __construct() {
        ...
        $this->ad_selector = VK_Adnetwork_Select::get_instance();
     ...
     *
     * @return    VK_Adnetwork    A single instance of this class.
     */
    ...
    /**
     * Return the VK_Adnetwork_Model responsible for loading ads and placements into the frontend
     *
     * @return VK_Adnetwork_Model
     */
    public function get_model() {
        ...
        if ( ! isset( $this->model ) ) {
            $this->model = new VK_Adnetwork_Model( $wpdb );
        }
    ...
    public function wp_plugins_loaded() {
        ...
        add_filter( 'vk-adnetwork-ad-types', [ $this, 'setup_default_ad_types' ], 5 );
     ...
     */
    public function vk_adnetwork_loaded() {
        do_action( 'vk-adnetwork-plugin-loaded' );
    }
    ...
    public function set_ad_types() {
        ...
        */
        $this->ad_types = apply_filters( 'vk-adnetwork-ad-types', $types );
    }
    ...
    public function set_disabled_constant() {
        ...
        if ( defined( 'VK_ADNETWORK_ADS_DISABLED' ) ) {
            return; }

        $options = $this->plugin->options();

        // check if ads are disabled completely.
        if ( ! empty( $options['vk-adnetwork-disabled-ads']['all'] ) ) {
        ...
        if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
            if ( ! empty( $options['vk-adnetwork-disabled-ads']['rest-api'] ) ) {
                $this->disable_ads();
            }
            return;
        }
        ...
        if ( $wp_the_query->is_404() && ! empty( $options['vk-adnetwork-disabled-ads']['404'] ) ) {
            $this->disable_ads( '404' );
            return;
        }

        // check if ads are disabled from non singular pages (often = archives).
        if ( ! $wp_the_query->is_singular() && ! empty( $options['vk-adnetwork-disabled-ads']['archives'] ) ) {
            $this->disable_ads( 'archive' );
            return;
        }

        // check if ads are disabled in Feed.
        if ( $wp_the_query->is_feed() && ( ! isset( $options['vk-adnetwork-disabled-ads']['feed'] ) || $options['vk-adnetwork-disabled-ads']['feed'] ) ) {
            $this->disable_ads();
            return;
        }
        ...
        if ( $wp_the_query->is_singular() && isset( $post->ID ) ) {
            $post_ad_options = get_post_meta( $post->ID, '_vk_adnetwork_ad_settings', true );
        ...
        if ( $wp_the_query->is_posts_page ) {
            $post_ad_options = get_post_meta( $wp_the_query->queried_object_id, '_vk_adnetwork_ad_settings', true );
        ...
        if ( function_exists( 'is_shop' ) && is_shop() ) {
            ...
            $shop_ad_options = get_post_meta( absint( $shop_id ), '_vk_adnetwork_ad_settings', true );
    ...
    private function disable_ads( $reason = null, $id = null ) {
        define( 'VK_ADNETWORK_ADS_DISABLED', true );
    ...
    public function setup_default_ad_types( $types ) {
        $types['plain']   = new VK_Adnetwork_Ad_Type_Plain(); /* plain text and php code */
        return $types;
    }
    ...
    public static function log( $message ) {
        if ( true === WP_DEBUG ) {
            if ( is_array( $message ) || is_object( $message ) ) {
                error_log( esc_html__( 'VK AdNetwork Error following:', 'vk-adnetwork' ) );
                error_log( print_r( $message, true ) );
            } else {
                // translators: %s is an error message generated by the plugin.
                $message = sprintf( esc_html__( 'VK AdNetwork Error: %s', 'vk-adnetwork' ), $message );
                error_log( $message );
            }
        }
    }
    ...
    public function update_options($options) {
        return $this->plugin->update_options($options);
    }
     ...
     *
     * Currently not in use.                        * В настоящее время не используется.
     * We do not use the location in the header.    * Мы не используем местоположение в хедере.
     *
     */
    public function inject_header() {
        $placements = get_option( 'vk_adnetwork-ads-placements', [] );
        if ( is_array( $placements ) ) {
            foreach ( $placements as $_placement_id => $_placement ) {
                if ( isset( $_placement['type'] ) && 'header' === $_placement['type'] ) {
                    ...
                    // phpcs:ignore
                    $header_content = VK_Adnetwork_Select::get_instance()->get_ad_by_method( $_placement_id, VK_Adnetwork_Select::PLACEMENT, $_options );
                    echo wp_kses( $header_content, wp_kses_allowed_html( 'post' ) + [ 'script' => true ] );
     ...
     *
     * footer // Нижний колонтитул // ad format id=109513 => [970x250]
     */
        ...
        if ( is_array( $placements ) ) {
            foreach ( $placements as $_placement_id => $_placement ) {
                if ( isset( $_placement['type'] ) && 'footer' === $_placement['type'] ) {
                    ...
                    // phpcs:ignore
                    echo wp_kses(
                            VK_Adnetwork_Select::get_instance()->get_ad_by_method( $_placement_id, VK_Adnetwork_Select::PLACEMENT, $_options ),
                            wp_kses_allowed_html( 'post' ) + [ 'script' => true ]
                    );
                }
            }
        }
    ...
    public function inject_content( $content = '' ) {
        ...
        // do not inject ads multiple times, e.g., when the_content is applied multiple times.  // не вводите объявления несколько раз, например, когда the_content применяется несколько раз.
        if ( $this->has_many_the_content() ) {
            return $content;
        }

        // Check if ads are disabled in secondary queries.                                      // Проверьте, отключена ли реклама во вторичных запросах.
        if ( ! empty( $options['vk-adnetwork-disabled-ads']['secondary'] ) ) {
            // this function was called by ajax (in secondary query).                           // эта функция была вызвана ajax (во вторичном запросе).
            if ( wp_doing_ajax() ) {
                return $content;
            }
            // get out of wp_router_page post type if ads are disabled in secondary queries.    // отключите тип публикации wp_router_page, если реклама отключена во вторичных запросах.
            if ( 'wp_router_page' === get_post_type() ) {
                return $content;
            }
        }

        // No need to inject ads because all tags are stripped from excepts.                    // Нет необходимости вставлять рекламу, потому что все теги удалены из исключений.
        if ( doing_filter( 'get_the_excerpt' ) ) {
            return $content;
        }

        // run only within the loop on single pages of public post types.                       // запускается только внутри цикла на отдельных страницах общедоступных типов записей.
        ...
        // make sure that no ad is injected into another ad.                                    // убедитесь, что ни одно объявление не вставлено в другое объявление.
        if ( get_post_type() === self::POST_TYPE_SLUG ) {
            return $content;
        }

        // Do not inject on admin pages.                                                        // Не вводите на страницах администратора.
        if ( is_admin() && ! defined( 'DOING_AJAX' ) ) {
            return $content;
        }

        // check if admin allows injection in all places.                                       // проверьте, разрешает ли администратор внедрение во всех местах.
        if ( ! isset( $options['content-injection-everywhere'] )
            || 0 === $options['content-injection-everywhere'] ) {
                    // check if this is a singular page within the loop or an AMP page.         // проверьте, является ли это единственной страницей в цикле или страницей AMP.
                    $is_amp = vk_adnetwork_is_amp();
        ...
        if ( ! apply_filters( 'vk-adnetwork-can-inject-into-content', true, $content, $placements ) ) {
            return $content;
        }
        ...
        if ( is_array( $placements ) ) {
            foreach ( $placements as $_placement_id => $_placement ) {
                ...
                // check if injection is ok for a specific placement ID.                        // проверьте, все ли в порядке с введением для определенного идентификатора размещения.
                if ( ! apply_filters( 'vk-adnetwork-can-inject-into-content-' . $_placement_id, true, $content, $_placement_id ) ) {
                    continue;
                }

                switch ( $_placement['type'] ) {
                    case 'post_top':
                        $content = VK_Adnetwork_Select::get_instance()->get_ad_by_method( $_placement_id, VK_Adnetwork_Select::PLACEMENT, $_options ) . $content;
                        break;
                    case 'post_bottom':
                        $content .= VK_Adnetwork_Select::get_instance()->get_ad_by_method( $_placement_id, VK_Adnetwork_Select::PLACEMENT, $_options );
                        break;
                    case 'post_content':
                        $content = VK_Adnetwork_Placements::inject_in_content( $_placement_id, $_options, $content );
                        break;
                }
            }
        }
        ...
        if ( ! empty( $_COOKIE['vk_adnetwork_frontend_picker'] ) ) {
            // Make possible to know where the content starts and ends.                         // Дайте возможность узнать, где начинается и заканчивается содержимое.
            $content = '<ins style="display: none;" class="vk_adnetwork-frontend-picker-boundary-helper"></ins>
            ' . $content;
        }
    ...
    public function can_display_ads() {
        ...
        if ( ! empty( $options['vk-adnetwork-disabled-ads']['secondary'] ) && ! $this->is_main_query() && ! wp_doing_ajax() ) {
            return false;
        }
    ...
    public function is_bot() {
        ...
        // показывайте рекламу в версии AMP также для ботов, чтобы позволить Google (и, возможно, другим) кэшировать страницу.
        ...
        $bots = apply_filters( 'vk-adnetwork-bots', $this->bots );
        $bots = implode( '|', $bots );
        // Make sure delimiters in regex are escaped.
        $bots = preg_replace( '/(.*?)(?<!\\\)' . preg_quote( '/', '/' ) . '(.*?)/', '$1\\/$2', $bots );

        return preg_match( sprintf( '/%s/i', $bots ), sanitize_text_field( wp_unslash( $_SERVER['HTTP_USER_AGENT'] ) ) );
    }
     ...
     * Проверьте, является ли текущий пользователь ботом, предварительно заполняющим кэш
     * Объявления должны быть загружены для бота, потому что они должны отображаться на кэшированном сайте
     *
     * @return bool
     */
    public function is_cache_bot() {
        if ( isset( $_SERVER['HTTP_USER_AGENT'] ) && '' !== $_SERVER['HTTP_USER_AGENT'] ) {
            ...
            // WP Rocket.
            if (str_contains($current, 'wprocketbot')) {
                return true;
            }
            ...
            if (str_contains($current, 'lscache_')) {
                return true;
            }
        }
    ...
    public function create_post_types() {
        if ( 1 !== did_action( 'init' ) && 1 !== did_action( 'uninstall_' . VK_ADNETWORK_BASE ) ) {
            return;
        }
    ...
    protected function get_post_type_params() {
        $labels = [
            'name'               => esc_html__( 'Ads',                      'vk-adnetwork' ),
            'singular_name'      => esc_html__( 'Ad',                       'vk-adnetwork' ),
            // Новый рекламный блок
            'add_new'            => esc_html__( 'New Ad',                   'vk-adnetwork' ),
            // Добавить новый рекламный блок
            'add_new_item'       => esc_html__( 'Add New Ad',               'vk-adnetwork' ),
            'edit'               => esc_html__( 'Edit',                     'vk-adnetwork' ),
            'edit_item'          => esc_html__( 'Edit Ad',                  'vk-adnetwork' ),
            // Новый рекламный блок
            'new_item'           => esc_html__( 'New Ad',                   'vk-adnetwork' ),
            'view'               => esc_html__( 'View',                     'vk-adnetwork' ),
            'view_item'          => esc_html__( 'View the Ad',              'vk-adnetwork' ),
            'search_items'       => esc_html__( 'Search Ads',               'vk-adnetwork' ),
            'not_found'          => esc_html__( 'No Ads found',             'vk-adnetwork' ),
            'not_found_in_trash' => esc_html__( 'No Ads found in Trash',    'vk-adnetwork' ),
            'parent'             => esc_html__( 'Parent Ad',                'vk-adnetwork' ),
        ];

        $supports = [ 'title', 'author' ];
        if ( defined( 'VK_ADNETWORK_ENABLE_REVISIONS' ) ) {
            $supports[] = 'revisions';
        }
        ...
        $post_type_params = [
            ...
            'capabilities' => [
                // Meta capabilities.
                'edit_post'              => 'vk_adnetwork_edit_ads',
                'read_post'              => 'vk_adnetwork_edit_ads',
                'delete_post'            => 'vk_adnetwork_edit_ads',
                'edit_page'              => 'vk_adnetwork_edit_ads',
                'read_page'              => 'vk_adnetwork_edit_ads',
                'delete_page'            => 'vk_adnetwork_edit_ads',
                // Primitive capabilities used outside of map_meta_cap().
                'edit_posts'             => 'vk_adnetwork_edit_ads',
                'publish_posts'          => 'vk_adnetwork_edit_ads',
                'read_private_posts'     => 'vk_adnetwork_edit_ads',
                // Primitive capabilities used within map_meta_cap().
                'read'                   => 'vk_adnetwork_edit_ads',
                'delete_posts'           => 'vk_adnetwork_edit_ads',
                'delete_private_posts'   => 'vk_adnetwork_edit_ads',
                'delete_published_posts' => 'vk_adnetwork_edit_ads',
                'edit_private_posts'     => 'vk_adnetwork_edit_ads',
                'edit_published_posts'   => 'vk_adnetwork_edit_ads',
                'create_posts'           => 'vk_adnetwork_edit_ads',
            ],
            'has_archive'  => false,
        ...
        return apply_filters( 'vk-adnetwork-post-type-params', $post_type_params );
    }
    ...
    public function custom_header_code(){
        if ( ! defined( 'VK_ADNETWORK_DISABLE_EDIT_BAR' ) && current_user_can( VK_Adnetwork_Plugin::user_cap( 'vk_adnetwork_edit_ads' ) ) ) {
            ?><style>
                div.vk_adnetwork-edit-bar{position:absolute;height:0;display:none;z-index:10000;animation:vk_adnetwork-edit-appear 2s linear 1;}
                @keyframes vk_adnetwork-edit-appear {
                    0% {opacity: 0.0;pointer-events: none;}
                    66% {opacity: 0.0;}
                    100% {opacity: 1.0;}
                }
                a.vk_adnetwork-edit-button{position:absolute;top:0;left:0;text-decoration:none !important;box-shadow:none;border-bottom:none;color:#0074a2;margin-top:-5px;}
                a.vk_adnetwork-edit-button span{top:10px;line-height:25px;margin-left:-5px;width:26px;height:26px;border-radius:13px;border:solid 1px #0074a2;background:#fff}
                <?php
                printf(
                    'div[class^="%s"]:hover > div.vk_adnetwork-edit-bar {display: inline-block; vertical-align: top;}',
                    // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                    esc_attr(VK_Adnetwork_Plugin::get_instance()->get_frontend_prefix())
                );
                ?>
                </style>
                <?php
    ...
    public function get_label( $placement_state = 'default' ) {
        ...
        $vk_adnetwork_options = self::get_instance()->options();

        if ( 'enabled' !== $placement_state && empty( $vk_adnetwork_options['custom-label']['enabled'] ) ) {
            return '';
        }

        $label = ! empty( $vk_adnetwork_options['custom-label']['text'] ) ? esc_html( $vk_adnetwork_options['custom-label']['text'] ) : esc_html_x( 'Advertisements', 'label above ads', 'vk-adnetwork' );

        $template = wp_kses(sprintf( '<div class="%s">%s</div>', VK_Adnetwork_Plugin::get_instance()->get_frontend_prefix() . 'adlabel', $label ), ['div' => ['class' => true]]);
        return apply_filters( 'vk-adnetwork-custom-label', $template, $label );
    }
