/*
 * vk adnetwork functions to be used directly within ad codes
 * функции рекламной сети вконтакте, которые будут использоваться непосредственно в рекламных кодах
 */
...
vk_adnetwork = {
    ...
    cookie_exists: function ( name ) {
        ...
        return c_value !== null && c_value !== "" && c_value !== undefined;
    ...
    move: function ( element, target, options ) {
        ...
        if ( target.length > 1 ) {
            console.log( "VK AdNetwork: element '" + target_string + "' found " + target.length + " times." );
        }
    ...
    privacy: {
        ...
        get_state: (
            function () {
                return function () {
                    ...
                    if ( window.vk_adnetwork_options.privacy.state !== 'unknown' ) {
                        // make sure this only gets executed once.
                        if ( ! vk_adnetwork.privacy.state_executed ) {
                            vk_adnetwork.privacy.state_executed = true;
                            vk_adnetwork.privacy.dispatch_event( window.vk_adnetwork_options.privacy.state, false );
                        }
                        return vk_adnetwork.privacy.state;
                    }

                    // If using the cookie method, fire an initial event, regardless if cookie set or not.
                    if ( window.vk_adnetwork_options.privacy['consent-method'] === 'custom' ) {
                        var cookie_regex = new RegExp( '.*?' + window.vk_adnetwork_options.privacy['custom-cookie-value'] + '[^;]*?' );
                        let cookie       = vk_adnetwork.get_cookie( window.vk_adnetwork_options.privacy['custom-cookie-name'] ) || '';

                        // Force the event, if we haven't yet fired one.
                        if ( ! vk_adnetwork.privacy.state_executed ) {
                            vk_adnetwork.privacy.state_executed = true;
                            vk_adnetwork.privacy.dispatch_event( cookie.match( cookie_regex ) ? 'accepted' : 'unknown', true );
                        }
                    }
                    ...
                    var cnt                = 0,
                        consentSetInterval = setInterval( function () {
                            ...
                            switch ( window.vk_adnetwork_options.privacy['consent-method'] ) {
                                case 'custom' :
                                    let cookie = vk_adnetwork.get_cookie( window.vk_adnetwork_options.privacy['custom-cookie-name'] ) || '';
                                    ...
                                    if ( cookie.match( cookie_regex ) ) {
                                        clearInterval( consentSetInterval );
                                        if ( vk_adnetwork.privacy.state !== 'accepted' ) {
                                            vk_adnetwork.privacy.dispatch_event( 'accepted', true );
                                        }
                                    }
                                    break;
                                    ...
                                    window.__tcfapi( 'addEventListener', 2, function ( TCData, listenerSuccess ) {
                                            ...
                                            if (
                                                ...
                                                if ( ! TCData.gdprApplies ) {
                                                    if ( vk_adnetwork.privacy.state !== 'not_needed' ) {
                                                        vk_adnetwork.privacy.dispatch_event( 'not_needed', userAction );
                                                    }
                                                    return;
                                                }
                                                ...
                                                if ( TCData.purpose.consents[1] ) {
                                                    if ( vk_adnetwork.privacy.state !== 'accepted' ) {
                                                        vk_adnetwork.privacy.dispatch_event( 'accepted', userAction );
                                                    }
                                                    return;
                                                }

                                                // fire another event, in case the user revokes the previous consent.
                                                if ( vk_adnetwork.privacy.state !== 'rejected' ) {
                                                    vk_adnetwork.privacy.dispatch_event( 'rejected', userAction );
                                                }
                                            }
                                        }
                                    );
        ...
        dispatch_event: function ( state, userAction ) {
            var previousState = vk_adnetwork.privacy.state,
                fire_event    = function () {
                    document.dispatchEvent( new CustomEvent( 'vk_adnetwork_privacy', {
                        detail: {
                            state: state,
            ...
            vk_adnetwork.privacy.state = state;
            ...
            window.vk_adnetwork_ready_queue.push( fire_event );
        },
        /**
...
window.vk_adnetwork_ready_queue.push( vk_adnetwork.privacy.get_state );

document.addEventListener( 'vk_adnetwork_privacy', function ( event ) {
    if (
        (
    ...
    document.querySelectorAll( 'script[type="text/plain"][data-tcf="waiting-for-consent"]' ).forEach( vk_adnetwork.privacy.decode_ad );
} );
