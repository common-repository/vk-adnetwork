 ...
 */
class VK_Adnetwork_In_Content_Injector {
    ...
    public static function &inject_in_content( $placement_id, $placement_opts, &$content, $options = [] ) {
        ...
        $tag = apply_filters( 'vk-adnetwork-placement-content-injection-xpath', $tag, $placement_opts );

        // get plugin options.
        $plugin_options = VK_Adnetwork::get_instance()->options();

        $defaults = [
            'allowEmpty'                   => false,
            'paragraph_select_from_bottom' => isset( $placement_opts['start_from_bottom'] ) && $placement_opts['start_from_bottom'],
            'position'                     => $placement_opts['position'] ?? 'after',
        ...
        $defaults['paragraph_id'] = $placement_opts['index'] ?? 1;
        ...
        $options = apply_filters(
            'vk-adnetwork-placement-content-injection-options',
        ...
        switch ( $tag_option ) {
            ...
            case 'headlines':
                $headlines = apply_filters( 'vk-adnetwork-headlines-for-ad-injection', [ 'h2', 'h3', 'h4' ] );
        ...
        $items = apply_filters( 'vk-adnetwork-placement-content-injection-items', $items, $xpath, $tag_option );
        ...
        if ( $options['paragraph_count'] >= $options['paragraph_id'] ) {
            ...
            $offsets    = apply_filters( 'vk-adnetwork-placement-content-offsets', [ $offset ], $options, $placement_opts, $xpath, $paragraphs, $dom );
            $did_inject = false;

            foreach ( $offsets as $offset ) {

                // inject.
                $node = apply_filters( 'vk-adnetwork-placement-content-injection-node', $paragraphs[ $offset ], $tag, $options['before'] );
                ...
                $ad_content = (string) VK_Adnetwork_Select::get_instance()->get_ad_by_method( $placement_id, 'placement', $placement_opts );
                ...
                // log errors.
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG && current_user_can( 'vk_adnetwork_manage_options' ) ) {
                    foreach ( libxml_get_errors() as $_error ) {
                        ...
                        if ( false === stripos( $_error->message, 'htmlParseEntityRef:' ) ) {
                            VK_Adnetwork::log( 'possible content injection error for placement "' . $placement_id . '": ' . print_r( $_error, true ) );
                        }
                    }
                }
             ...
             */
        } elseif ( current_user_can( VK_Adnetwork_Plugin::user_cap( 'vk_adnetwork_manage_options' ) )
                && $options['itemLimit'] !== -1
            ...
            if ( $options['paragraph_id'] <= count( $paragraphs ) ) {
                ...
                add_filter( 'vk-adnetwork-ad-health-nodes', [ 'VK_Adnetwork_In_Content_Injector', 'add_ad_health_node' ] );
            }
        }
    ...
    private static function get_content_to_load( $content, $wp_charset ) {
        ...
        if ( $wpautop_priority && VK_Adnetwork_Plugin::get_instance()->get_content_injection_priority() < $wpautop_priority ) {
    ...
    private static function filter_ad_content( $ad_content, $tag_name, $options ) {
        ...
        return '%vk_adnetwork_placeholder_' . $id . '%';
    }
    ...
    private static function inject_ads( $content, $content_orig, $ads_for_placeholders ) {
        ...
        unset( $ad_content );
        usort( $ads_for_placeholders, [ 'VK_Adnetwork_In_Content_Injector', 'sort_ads_for_placehoders' ] );
        ...
        // Add ad placeholder.
        $alts[]                     = '%vk_adnetwork_placeholder_(?:\d+)%';
        ...
        foreach ( $tag_matches[0] as $r ) {
            if ( preg_match( '/%vk_adnetwork_placeholder_(\d+)%/', $r, $result ) ) {
                $id       = $result[1];
                $found_ad = false;
    ...
    public static function add_ad_health_node( $nodes ) {
        $nodes[] = [
            ...
            'data' => [
                ...
                'id'     => 'vk_adnetwork_ad_health_the_content_not_enough_elements',
                'title'  => sprintf(
                /* translators: %s stands for the name of the "Disable level limitation" option and automatically translated as well */
                    wp_kses(__( 'Set <em>%s</em> to show more ads', 'vk-adnetwork' ), ['em' => true]),
                    esc_html__( 'Disable level limitation', 'vk-adnetwork' )
                ),
                'href'   => admin_url( '/admin.php?page=vk-adnetwork-settings#top#general' ),
                'meta'   => [
    ...
    private static function get_ancestors_to_limit_query() {
         ...
         */
        $items = apply_filters(
            'vk-adnetwork-content-injection-nodes-without-ads',
            [
                [
                    // a class anyone can use to prevent automatic ad injection into a specific element.
                    'node' => '.vk_adnetwork-stop-injection',
                    'type' => 'ancestor',
                ],
                [
