 ...
 *
 * vk_adnetwork_push_notice() function to push notifications using AJAX in admin/assets/js/admin-global.js
 * push_ad_health_notice() in VK_Adnetwork_Ad_Ajax_Callbacks to push notifications sent via AJAX
 * VK_Adnetwork_Checks – for the various checks
 * list of notification texts in admin/includes/ad-health-notices.php
 */
class VK_Adnetwork_Ad_Health_Notices {
     ...
     * @const string
     */
    const DAILY_CHECK_TRANSIENT_NAME = 'vk-adnetwork-daily-ad-health-check-ran';
    ...
    public function __construct() {
        ...
        if ( [] === $this->default_notices ) {
            include VK_ADNETWORK_BASE_PATH . 'admin/includes/ad-health-notices.php';
            /**
             * @var $vk_adnetwork_ad_health_notices
             */
            $this->default_notices = $vk_adnetwork_ad_health_notices;
        }
        ...
        add_action( 'vk-adnetwork-ad-expired', [ $this, 'ad_expired' ], 10, 2 );
    }
    ...
    public function load_notices() {
        ...
        if ( isset( $_GET['vk_adnetwork_nonce'] ) && wp_verify_nonce( sanitize_text_field(wp_unslash($_GET['vk_adnetwork_nonce'])), 'vk-adnetwork-show-hidden-notices' )
            && isset( $_GET['vk_adnetwork-show-hidden-notices'] ) && 'true' === $_GET['vk_adnetwork-show-hidden-notices'] ) {
            $this->unignore();
    ...
    public function remove_query_vars_after_notice_update( $removable_query_args ) {
        $removable_query_args[] = 'vk_adnetwork-show-hidden-notices';
        $removable_query_args[] = 'vk_adnetwork_nonce';
     ...
     * - on each VK AdNetwork related page
     */
    public function run_checks() {

        // run in WP Admin only and if there are any ads.
        if ( ! is_admin() || ! VK_Adnetwork::get_number_of_ads() ) {
            return;
        }
        ...
        // run only daily unless we are on an VK AdNetwork related page.
    ...
    /**
     * General checks done on each VK AdNetwork-related page or once per day
     */
    public function checks() {

        if ( ! VK_Adnetwork_Checks::php_version_minimum() ) {
            $this->add( 'old_php' );
        } else {
            $this->remove( 'old_php' );
        }
        if ( count( VK_Adnetwork_Checks::conflicting_plugins() ) ) {
            $this->add( 'conflicting_plugins' );
        } else {
            $this->remove( 'conflicting_plugins' );
        }
        if ( count( VK_Adnetwork_Checks::php_extensions() ) ) {
            $this->add( 'php_extensions_missing' );
        } else {
            $this->remove( 'php_extensions_missing' );
        }
        if ( VK_Adnetwork_Checks::ads_disabled() ) {
            $this->add( 'ads_disabled' );
        } else {
            $this->remove( 'ads_disabled' );
        }
        if ( VK_Adnetwork_Checks::get_defined_constants() ) {
    ...
    public function ignore( $notice_key ) {
        ...
        if (!in_array($notice_key, $ignored, true)) {
            $ignored[] = $notice_key;
        }
    ...
    public function render_widget() {
        ...
        if ( $this->has_notices() ) {
            include VK_ADNETWORK_BASE_PATH . 'admin/views/overview-notices.php';
        }
    }
    ...
    public function display( $type = 'problem' ) {
        ...
        ?>
        <ul class="vk_adnetwork-ad-health-notices vk_adnetwork-ad-health-notices-<?php echo esc_attr( $type ); ?>">
            <?php
            ...
            foreach ( $this->notices as $_notice_key => $_notice ) {
                ...
                $notice_type = $notice_array['type'] ?? 'problem';
                ...
                $can_hide  = ! isset( $notice_array['can_hide'] ) || true === $notice_array['can_hide'];
                $hide      = ! isset( $notice_array['hide'] ) || true === $notice_array['hide'];
                $is_hidden = in_array( $_notice_key, $this->ignore, true );
                $date      = isset( $_notice['time'] ) ? date_i18n( get_option( 'date_format' ), $_notice['time'] ) : false;

                include VK_ADNETWORK_BASE_PATH . 'admin/views/overview-notice-row.php';
            }
    ...
    public function options() {
        if ( ! isset( $this->options ) ) {
            $this->options = get_option( VK_ADNETWORK_SLUG . '-ad-health-notices', [] );
        }
    ...
    public function update_options( array $options ) {
        ...
        update_option( VK_ADNETWORK_SLUG . '-ad-health-notices', $options );
    }
    ...
    public function get_visible_notices_by_type( $type = 'problem' ) {
        ...
        foreach ( $this->notices as $_key => $_notice ) {
            ...
            if ( isset( $notice_array['type'] ) && $type === $notice_array['type']
                 && ( ! isset( $this->ignore ) || !in_array($_key, $this->ignore, true)) ) {
    ...
    public function get_notice_array_for_key( $notice_key ) {
        ...
        $orig_key = $this->notices[$notice_key]['orig_key'] ?? $notice_key;

        return $this->default_notices[$orig_key] ?? [];
    }
    ...
    public function get_help_link( $link ) {
        ...
        return '&nbsp;<a href="' . $link . '" target="_blank">' . esc_html__( 'Get help', 'vk-adnetwork' ) . '</a>';
    }
