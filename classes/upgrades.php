 ...
 */
class VK_Adnetwork_Upgrades {
    
    public function __construct(){

        $internal_options = VK_Adnetwork_Plugin::get_instance()->internal_options();

        // the 'vk_adnetwork_edit_ads' capability was added to POST_TYPE_SLUG post type in this version
        if ( ! isset( $internal_options['version'] ) || version_compare( $internal_options['version'], '1.7.2', '<' ) ) {
            VK_Adnetwork_Plugin::get_instance()->create_capabilities();
        }
        ...
        if( ! $suppress_version_number_update ){
            VK_Adnetwork_Admin_Notices::get_instance()->update_version_number();
        }
    }
}